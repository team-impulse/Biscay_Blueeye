...
def processdata(received):
    ...
    try:
        ...
        latest[5] = ((int(split[11],16)<<24)|(int(split[12],16)<<16)|(int(split[13],16)<<8)|(int(split[14],16)))/10000.0
        latest[6] = ((int(split[15],16)<<24)|(int(split[16],16)<<16)|(int(split[17],16)<<8)|(int(split[18],16)))/10000.0
        latest[6]*=const_lng_mult
        latest[5]+=50
        latest[7] = int(split[7],16)<<8 | int(split[8],16)
        latest[8] = int(split[9],16)<<8 | int(split[10],16)

        last_status[3]=True if (int(split[19],16)&0x1) == 1 else False
        last_status[2] = True if (int(split[19],16)&0x2)>>1 ==1 else False
        last_status[1] = True if (int(split[19],16)&0x4)>>2==1 else False
        last_status[0] = True if (int(split[19],16)&0x8)>>3==1 else False
